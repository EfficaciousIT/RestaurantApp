...
import com.mobi.efficacious.esmartresatarant.adaptors.ViewCategoryAdapter;
...
public class ViewCategoryActivity extends FragmentActivity implements SearchView.OnQueryTextListener, EditOrderDetailsActivity.EditAgeDialogListener {
	GridView gridView;
	String RESTAURANTID;
	ArrayList<Categories> categories = new ArrayList<Categories>();
	ViewCategoryAdapter adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext=ViewCategoryActivity.this;
		
		settings = getSharedPreferences(PREFRENCES_NAME,Context.MODE_PRIVATE);
		RESTAURANTID = settings.getString("TAG_RESTAURANTID", "");
		alert = new AlertDialogManager();
		cd = new ConnectionDetector(getApplicationContext());
		gridView = (GridView)findViewById(R.id.gridView_viewcategoty);
		
		searchView = (SearchView)findViewById(R.id.search_viewcategory);
		PostData();
	...
	{ 
		if (!cd.isConnectingToInternet()) 
		 {
			alert.showAlertDialog(ViewCategoryActivity.this,"No Internet Connection","Please connect to working Internet connection", false);
			return;
		 }
		 else
		 {
		        ...
		        if(!(envelope.bodyIn instanceof SoapFault))
		        {
		        	...
		        	if(response != null)
		        	{
		        		result=response.getProperty(0).toString(); 
		            	  ...
		            	  adapter = new ViewCategoryAdapter(mContext, categories);
		        	...
		        	{
		        		Toast.makeText(ViewCategoryActivity.this, "Null Response",Toast.LENGTH_LONG).show();
		        	}
		        }
		        else
		        {
		        	Toast.makeText(ViewCategoryActivity.this, "Error",Toast.LENGTH_LONG).show();
		        }			          
	        }	       
			catch (Exception e) 
			{		
	...
	@Override
	public void EditAgeDialogResult(String inputText) {
